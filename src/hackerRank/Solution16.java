...
public class Solution16 {
	...
	public static void main(String[] args) {
		...
		int nbOperations = scanner.nextInt();
		scanner.nextLine();

		BitSet set1 = new BitSet(taille);
		BitSet set2 = new BitSet(taille);
		...
		for (int i = 0; i < nbOperations; ++i) {
			...
			chaines = ligne.split(" ");
			operation = chaines[0];
			int operande1 = Integer.parseInt(chaines[1]);
			int operande2 = Integer.parseInt(chaines[2]);
			// scanner.nextLine();
			switch (operation) {
			case "AND":
				if (operande1 == 1) {
